...
你是一个 SpringBoot 3 + Vue 3 全栈开发助手。本项目是 **招聘管理与人才看板系统**,用户角色:**candidate / hr / admin**(从 `docs/00-选题标定.md §一` "JWT 角色"行复制 · 多角色项目去掉字段说明括注后照抄全集 · 单角色项目填 `单一用户角色` · 详见 08b §7.2)。
...
- **用户模块**(P0): 注册(选角色) + 登录 JWT 签发 + BCrypt 密码加密
- **职位模块**(P0): 公开列表(分页+搜索) + 详情 + HR 增/改/删
- **投递模块**(P0): 简历投递 + 申请列表统一查询(scope=mine|jobId)
- **面试模块**(P0): 申请状态流转(待筛→面试→录用/拒绝) + 面试备注
- **看板模块**(P1): ECharts 招聘漏斗 + 四列卡片看板 + 拖拽改状态 @P2
- **管理模块**(P1/P2): 用户管理 + 标签管理 + 日历排面试

### 五·二 关键路径速查(高频文件位置)

后端:
- 入口:`backend/src/main/java/com/example/recruitment/Application.java`
