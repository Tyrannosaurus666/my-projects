-- 数据库初始化脚本 · 项目: 招聘管理与人才看板系统 · 生成时间: 2026-05-20
-- 数据库名: recruitment_db (从 application.yml datasource.url 提取)
...
CREATE TABLE job_post (
  ...
  requirements TEXT         NULL COMMENT '岗位要求(TEXT类型·上限2000字符)',
  status      VARCHAR(20)   NOT NULL DEFAULT '招聘中' COMMENT '职位状态:招聘中/停招',
  hr_id       BIGINT        NOT NULL COMMENT '发布者HR的用户ID(FK→user.id)',
  deadline    DATETIME      NULL COMMENT '截止日期(NULL=长期有效)',
  is_deleted  TINYINT(1)    NOT NULL DEFAULT 0 COMMENT '逻辑删除:0=正常,1=已删除',
  create_time DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE application (
  ...
  status      VARCHAR(20)   NOT NULL DEFAULT '待筛选' COMMENT '申请状态:待筛选/已面试/已录用/已拒绝',
  resume_url  VARCHAR(255)  NOT NULL COMMENT '简历文件路径',
  resume_hash VARCHAR(32)   NULL COMMENT '简历文件MD5哈希',
  resume_text TEXT          NULL COMMENT '简历文本提取内容',
  is_deleted  TINYINT(1)    NOT NULL DEFAULT 0 COMMENT '逻辑删除:0=正常,1=已删除',
  create_time DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间(投递时间)',
  update_time DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  UNIQUE INDEX uniq_job_user (job_id, user_id) COMMENT '防重复投递',
  INDEX idx_job_id (job_id),
  INDEX idx_user_id (user_id),
  INDEX idx_status (status),
  FOREIGN KEY (job_id) REFERENCES job_post(id) ON DELETE RESTRICT,
...
CREATE TABLE job_tag (
  ...
  create_time DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  UNIQUE INDEX uniq_job_tag (job_id, tag_name),
...
-- 表 6: application_stage
CREATE TABLE application_stage (
  id          BIGINT        AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
  app_id      BIGINT        NOT NULL COMMENT '投递记录ID(FK→application.id)',
  from_status VARCHAR(20)   NULL COMMENT '变更前状态(NULL=首次创建)',
  to_status   VARCHAR(20)   NOT NULL COMMENT '变更后状态',
  operator_id BIGINT        NOT NULL COMMENT '操作人用户ID(FK→user.id)',
  legacy      TINYINT(1)    NOT NULL DEFAULT 0 COMMENT '遗留标记:0=新记录,1=P0遗留逆序记录',
  create_time DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  update_time DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  INDEX idx_app_id (app_id),
...
CREATE TABLE interview_schedule (
  ...
  update_time    DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  INDEX idx_app_id (app_id),
  INDEX idx_hr_time (hr_id, interview_time),
  FOREIGN KEY (app_id) REFERENCES application(id) ON DELETE RESTRICT,
  FOREIGN KEY (hr_id) REFERENCES user(id) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='面试日程表(P2·可选)';

-- ============================================================
-- 测试数据(按外键依赖顺序:user→job_post→application→interview_note)
-- ============================================================

-- 密码均为 123456 的 BCrypt 哈希(教学测试用)
INSERT INTO user (username, password, role) VALUES
('candidate01', '$2a$10$EixZaYVK1fsbw1ZfbX3OXePaWxn96p36WQoeG6Lruj3vjPGga31lW', 'candidate'),
('candidate02', '$2a$10$EixZaYVK1fsbw1ZfbX3OXePaWxn96p36WQoeG6Lruj3vjPGga31lW', 'candidate'),
('hr01',        '$2a$10$EixZaYVK1fsbw1ZfbX3OXePaWxn96p36WQoeG6Lruj3vjPGga31lW', 'hr'),
('hr02',        '$2a$10$EixZaYVK1fsbw1ZfbX3OXePaWxn96p36WQoeG6Lruj3vjPGga31lW', 'hr'),
('admin01',     '$2a$10$EixZaYVK1fsbw1ZfbX3OXePaWxn96p36WQoeG6Lruj3vjPGga31lW', 'admin');
